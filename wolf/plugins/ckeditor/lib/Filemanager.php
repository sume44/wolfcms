...
class Filemanager {
    ...
    public function __construct($config = array(), $extraConfig = array())
    {            
        $this->config = $config;
        ...
        if (isset($this->config['options']['root']) && $this->config['options']['root'] !== false ) {
            $this->root = $this->config['options']['root'];
        } else {
            $this->root = dirname(dirname(dirname(__FILE__))).DIRECTORY_SEPARATOR;
        }

        if (isset($this->config['options']['fileConnector']) && $this->config['options']['fileConnector'] !== false ) {
            $this->fileConnector = $this->config['options']['fileConnector'];
        } else {
            $this->fileConnector = 'connectors/php/filemanager.php';
        }
    ...
    private function get_file_info($path='', $thumbnail = false) {
        ...
        if(is_dir($current_path)) {
            ...
            if($this->item['filetype'] == 'svg') {
                ...
                $this->item['preview'] = $this->fileConnector.'?mode=preview&path='. rawurlencode($current_path);
    ...
    private function get_thumbnail($path) {
        ...
        require_once('vendor/Intervention/Image/Image.php');
        ...
        $thumbnail_path = $a[0].$this->separator.DIRECTORY_SEPARATOR.$this->cachefolder.dirname(end($a));
        ...
        if ( ! file_exists($thumbnail_fullpath)) {
            ...
            $image->resize($this->thumbnail_width, $this->thumbnail_height);

            $image->save($thumbnail_fullpath, 75);
